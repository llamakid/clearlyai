...
- ⏸️ **AEO audit tool + AI Visibility Tracker spun out (2026-07-28)** — the "AI Search Checkup" (`/tools/audit`) and its companion "AI Visibility Tracker" (`/tools/tracker`, weekly cron re-crawls + score-drop email alerts) were removed from this app to become their own standalone paid product. Removed: `lib/aeo/` engine, `app/tools/audit/`, `app/(protected)/tools/tracker/`, `app/api/tracker/`, `app/api/cron/visibility-tracker/`, `components/tools/ReportCard.tsx` + `TrackerView.tsx`, the `cheerio` dependency, `visibilityAlertEmail()` in `lib/email-drip.ts`, and the cron entry in `vercel.json`. The `tracked_sites`/`visibility_snapshots` tables are dropped via `supabase-migration-remove-aeo-tracker.sql` (run manually once the standalone product is live). `/tools/write` and `/tools/explain` are unaffected — they use the same `tool_usage`/`anon_tool_usage` rate-limit tables but have no other AEO coupling. `app/tools/page.tsx` has a TODO to add a cross-promo card linking to the new standalone product once it has a domain.
...
- ✅ Google Sign-In live (2026-07-04) — `GoogleSignInButton.tsx` on `/login`, OAuth client set up in Google Cloud Console, provider enabled in Supabase. Callback route (`app/api/auth/callback/route.ts`) now checks `exchangeCodeForSession` errors explicitly and redirects to `/login?error=...` instead of silently proceeding — fixes a first-attempt failure mode seen in local testing where a failed exchange bounced users out with no explanation. OAuth consent screen may still be in Testing mode in Google Cloud — confirm it's published before promoting Google sign-in broadly.
